...
- **The X result cap is a per-account count, and a count cap truncates unevenly.** The actor bills per tweet against a $5/month free plan, so `X_RESULTS_LIMIT` exists to bound cost. It returns **newest first**, so hitting it drops the *oldest* posts in the window rather than sampling at random — a known, directional truncation. But because the cap is on **count** while the accounts post at very different rates, the same cap buys very different spans of history. At the original 160:
  ...
  **This produced a false finding before it was caught, and it is the sharpest lesson in the project.** Cadence divides every account by one shared window, so all four came out at exactly 17.11 posts/week and the dashboard reported `principalVsPeers = 1.00×` — "he posts exactly as often as his peers". That number measured the result cap, not the accounts. Consistency was worse: the principal read as posting in 30% of weeks when he posts most days, because seven of the ten blocks predated anything that had been fetched.

  Three things now stand between that data and that claim. `compareCadence` **detects** that an account's history does not cover the window it is being divided by. It then **re-measures over the window every account does cover**, stating the shortened span and who forced it — on the capped corpus that recovered the real finding, 2.19× the peer median, in place of the manufactured 1.00×. And where even that span is too short to be worth reporting (under two weeks) or still has a silent account in it, it **withholds the platform's cadence figures entirely** — the comparison *and* the principal's own rate — with a sentence saying why.

  **The cap itself was also simply wrong, and cheap to fix.** At $0.00025/tweet, covering the full 90 days for all four accounts is ~2,400 tweets ≈ **$0.60** — inside the free plan, not a paid-plan problem as this section previously claimed. `X_RESULTS_LIMIT=1500` covers 94% of the window for the principal and X now reports cadence over the same 90 days as every other platform. The narrowing logic stays as the guard for the next roster whose busiest account outgrows the cap.

  Engagement-rate analysis on X was **never** affected either way: rate is per post, so a shorter history is a smaller sample rather than a distorted one.
